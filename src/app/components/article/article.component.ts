    ...
    if (this.articleId) {
      ...
      const token = localStorage.getItem('token'); // Récupère le token de l'utilisateur
        if (!token) {
            ...
            return; // Annule la requête si le token est manquant
        }

        const headers = { Authorization: `Bearer ${token}` }; // Ajoutez le token dans les headers
      // Récupérer les données de l'article
      this.http
        .get(`http://127.0.0.1:8000/api/articles/${this.articleId}`, { headers })
        .subscribe(
          (response: any) => {
            this.likes = response.likes;
            this.liked = response.userLiked;
            this.commentsCount = response.comments_count;
          },
          (error) => {
            console.error(
